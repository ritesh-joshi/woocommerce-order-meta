<?php 
  ...
  function my_field_order_meta_handler( $item_id, $values, $cart_item_key ) {
    ...
    if( isset( $values['design'] ) ) {
	 ...
	 if( isset( $values['general_information'] ) ) {
        wc_add_order_item_meta( $item_id, "general_information", $values['general_information']);
		}

	if( isset( $values['attachment'] ) ) {
        wc_add_order_item_meta( $item_id, "attachment", $values['attachment']);
		}		
	}
} 
